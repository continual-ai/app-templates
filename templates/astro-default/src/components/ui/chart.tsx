...
function ChartTooltipContent({
  ...
  return (
    <div
      ...
      className={cn(
        "grid min-w-32 gap-2 rounded-lg border bg-popover px-3 py-2 text-sm text-popover-foreground shadow-md",
        className
      )}
    >
            ...
            <div key={key} className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 text-muted-foreground">
                <span
                  ...
                  style={{ background: item.color ?? itemConfig?.color ?? `var(--color-${key})` }}
                />
...
function ChartLegendContent({
  ...
  return (
    <div data-slot="chart-legend" className="flex flex-wrap items-center justify-center gap-4 text-sm">
      {payload.map((item) => {
