...
For new database-backed Apps, prefer `@neondatabase/serverless`: use its HTTP `neon()` client for
one-shot queries and non-interactive transactions, and its WebSocket-compatible `Pool` only for
interactive transactions or `node-postgres` compatibility. Add the exact dependency when the App
first needs the database so database-free Apps do not carry an unused driver.

Database clients are request-scoped. Create the client inside the server handler, close a `Pool`
before the handler returns, and never cache a client, pool, connection promise, or connection string
in module state. The publisher-provided URL uses Neon's transaction pooler, so prefer
schema-qualified queries instead of session-level `SET search_path`.
